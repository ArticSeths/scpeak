<template>
  <section id="download" class="py-20 px-4">
    <div class="max-w-4xl mx-auto">
      <p class="text-xs tracking-[0.2em] text-primary-400 uppercase mb-4 text-center">Install</p>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-10">
        <a
          ...
          :key="d.os"
          :href="d.url"
          class="p-5 rounded-xl border border-surface-800 bg-surface-900 hover:border-surface-600 transition-colors group"
        >
          <p class="text-xs text-surface-500 uppercase tracking-wider mb-3">{{ d.os }}</p>
          <span class="text-2xl">{{ d.icon }}</span>
          <p class="text-sm text-surface-300 mt-2">{{ d.label }}</p>
          <p class="text-xs text-primary-400 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
            Descargar →
          </p>
        </a>
      </div>

      <p class="text-center mt-8 text-xs text-surface-600">
        Versión {{ release?.tag_name || "..." }} —
        <a href="https://github.com/ArticSeths/scpeak/releases" target="_blank" class="text-primary-400 hover:underline">
